...
async function preselectFromUrl(){
    ...
    // after package loads, pre-select tier if passed
    if(tierIndex !== null){
        // slight delay to let tier radios render
        setTimeout(() => {
...
// ---------- Package change — load dates + tiers ----------
...
packageSelect.addEventListener("change", async () => {
    ...
    travelDateSelect.innerHTML = `<option value="">Select Travel Date</option>`;
    ...
    bookingAmountInput.value = data.bookingAmount || "";

    // Travel dates
    if(data.travelDates){
        data.travelDates.forEach(date=>{
            travelDateSelect.innerHTML +=
                `<option value="${date.start} to ${date.end}">${date.start} - ${date.end}</option>`;
        });
    }

    // Price tiers
    const tiers = data.priceTiers && data.priceTiers.length
        ? data.priceTiers
        : [{ label: "Standard", price: Number(data.price) || 0 }];
        ...
        tierWrap.innerHTML = `
            <div class="fg">
                ...
                <select id="tierSelect">
                    ${tiers.map((t,i) =>
                        `<option value="${i}" data-label="${t.label}" data-price="${t.price}">
                            ${t.label} — ₹${t.price}
                        </option>`
                    ).join("")}
                </select>
    ...
    try{
        ...
        if(tierSelect){
            const selectedOption =
                tierSelect.options[tierSelect.selectedIndex];
            tierLabel = selectedOption.dataset.label || selectedOption.text;
            tierPrice = Number(selectedOption.dataset.price) || 0;
        }
        ...
        const booking = {
            ...
            packageTitle: packageSelect.options[packageSelect.selectedIndex].text,
            travelDate: travelDate.value,
            passengers: passengers.value,
            bookingAmount: bookingAmountInput.value,
            tierLabel: tierLabel,
            tierPrice: tierPrice,
        ...
        if(!booking.customerName || !booking.phone || !booking.packageId){
